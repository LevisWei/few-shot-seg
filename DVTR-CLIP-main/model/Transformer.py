...
class MixVisionTransformer(BaseModule):
    def __init__(self,
        ...
        if self.shot == 1:
            conv_channel = self.match_dims + 2 * self.num_similarity_channels + 2
        else:
            conv_channel = self.match_dims + 2 * self.num_similarity_channels + 8 + 2
    ...
    def forward(self, q_x, s_x, mask, similarity, ori_similarity, cam_map):
        down_query_features = []
        ...
        for i, layer in enumerate(self.down_sample_layers):
            ...
            tmp_cam = resize(cam_map, q_hw_shape, mode="bilinear", align_corners=True)
        ...
        for i in range(self.num_down_stages).__reversed__():
            ...
            out = layer[1](torch.cat([out, down_similarity[i], down_similarity_ori[i], down_cam[i]], dim=1))
...
class Transformer(nn.Module):
    ...
    def forward(self, features, supp_features, mask, similaryty, ori_similarity, cam_map):
        shape = features.shape[-2:]
        outs, weights = self.mix_transformer(features, supp_features, mask, similaryty, ori_similarity, cam_map)
        return outs, weights