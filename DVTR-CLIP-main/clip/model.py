...
def get_similarity(q, s, mask, attn):
    ...
    similarity = torch.bmm(tmp_supp.float(), tmp_query.float()) / (
                torch.bmm(tmp_supp_norm.float(), tmp_query_norm.float()) + cosine_eps)
    ...
    cam_refined = cam_refined.reshape(int(math.sqrt(cam_refined.shape[0])), int(math.sqrt(cam_refined.shape[0])),
                                      int(cam_refined.shape[1]))
...
class CLIP(nn.Module):
    ...
    def encode_text_with_visual(self, text, visual_feature):
        pos_x, pos_y = torch.where(text == 539)
        visual_feature = visual_feature.unsqueeze(1)  # [B, 1, D]
        x = self.token_embedding(text).type(self.dtype)  # [B, L, D]


        x_new = torch.zeros_like(x).to(x.device)
        for i in range(x.shape[0]):
            x_new[i, :, :] = torch.cat([x[i, :(pos_y[i]), :],
                                        visual_feature[i, :, :],
                                        x[i, (pos_y[i]+1):(self.context_length - visual_feature.shape[1] + 1), :]], dim=0)
        x_new = x_new + self.positional_embedding.type(self.dtype)
        x_new = x_new.permute(1, 0, 2)
        x_new, _ = self.transformer(x_new)
        x_new = x_new.permute(1, 0, 2)
    ...
    def encode_text_with_context(self, text, context):
        x_text = self.token_embedding(text).type(self.dtype)
        B, N1, C = x_text.shape
        _, N2, _ = context.shape
    ...
    def forward_last_layer(self, image_features, text_features):
        ...
        image_features = x
        # print(f"img_feat size: {image_features.size()}")
