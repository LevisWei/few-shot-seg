...
def get_parser():
    ...
    # parser.add_argument("--deep_prompt_len", type=int, default=2, help="the length of the learnable text embeddings n ")
...
def validate(val_loader, model, val_seed):
    ...
    for e in range(db_epoch):
        for i, (input, input_name, target, target_b, s_input, s_mask, subcls, class_name, ori_label, ori_label_b, img_cv2) in enumerate(val_loader):
            ...
            # output, meta_out, base_out = model(s_x=s_input, x_cv2=img_cv2, que_name=input_name, s_y=s_mask, x=input, y_m=target, y_b=target_b, cat_idx=subcls, class_name=class_name)
            output = model(s_x=s_input, x_cv2=img_cv2, que_name=input_name, s_y=s_mask, x=input, y_m=target, y_b=target_b, cat_idx=subcls, class_name=class_name)
