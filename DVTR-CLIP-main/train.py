...
def get_parser():
    ...
    # parser.add_argument("--deep_prompt_len", type=int, default=4, help="the length of the learnable text embeddings n ")
    # parser.add_argument("--total_d_layer_len", type=int, default=12, help="number of layers for the text encoder with learnable text embeddings")
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    for i, (input, input_name, target, target_b, s_input, s_mask, subcls, class_name, img_cv2) in enumerate(train_loader):
        ...
        # print("grad:", model.clip_model.deep_prompt_embeddings.grad.norm())

        # for name, param in model.context_decoder.named_parameters():
        #     if param.grad is not None:
        #         print(
        #             f"[GRAD] {name}: mean={param.grad.abs().mean().item():.6f}, max={param.grad.abs().max().item():.6f}")
        #     else:
        #         print(f"[GRAD] {name}: No grad!")

        # print("logit_scale =", model.logit_scale.exp().item())
        # print("context grad norm:", model.contexts.grad.norm().item())

        # for name, param in model.named_parameters():
        #     if param.grad is not None:
        #         if torch.isnan(param.grad).any():
        #             print(f"NaN detected in gradient for {name}. Stopping training.")
        #             exit(1)
        #         else:
